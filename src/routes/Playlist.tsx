  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col gap-5 sm:flex-row sm:items-end">
        <Artwork src={artwork} alt={title} className="h-44 w-44 shadow-2xl" rounded="rounded-2xl" />
        <div className="min-w-0 flex-1">
          <p className="text-xs font-medium tracking-[0.2em] text-ink-400 uppercase">
            {local ? 'Your playlist' : 'Playlist'}
          </p>
          <h1 className="mt-1.5 text-3xl font-bold tracking-tight text-white sm:text-4xl">{title}</h1>
          {collection?.description && (
            <p className="mt-2 line-clamp-2 text-sm text-ink-400">{collection.description}</p>
          )}
          ...
          {list.length > 0 && (
            <div className="mt-5 flex gap-2">
              ...
              <Button
                variant="outline"
                onClick={() => void playQueue(list, Math.floor(Math.random() * list.length))}
              >
                <Shuffle size={15} />
                Shuffle
              </Button>
